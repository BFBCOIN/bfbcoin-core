...
`test/functional/test_framework/test_framework.py:bfbTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/bfb_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_bfbd = config["components"].getboolean("ENABLE_BFBD")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/bfbProject/bfb/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/bfbProject/bfb/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_utils and enable_bfbd):
        print("No functional tests to run. Wallet, utils, and bfbd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    # Warn if bfbd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "bfbd"]) is not None:
            print("%sWARNING!%s There is already a bfbd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "BFBD" not in os.environ:
        os.environ["BFBD"] = build_dir + '/src/bfbd' + exeext
        os.environ["BFBCLI"] = build_dir + '/src/bfb-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        # In case there is a graveyard of zombie bfbds, we can apply a
...
class RPCCoverage():
    ...
    commands per `bfb-cli help` (`rpc_interface.txt`).
