...
The RPM spec file provided here is for bfb-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bfb.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[bfb git repository](https://github.com/bfbProject/bfb) but are not part of
...
Currently only `Source10` is used, for the example `bfb.conf` file.
...
in the bfb git repository. At present only an SVG version of the bfb
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bfb
...
needed when 0.12.1 is released, a proper fix is already in the bfb git
master branch.
...
idea to build bfb Core against a static version of that library compiled
...
The `bfb.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bfb-server` package. At this point, most RPM based
...
The `bfb-server` package should have SELinux support. How to properly do
...
bfb-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bfb/). He has been
packaging bfb for Fedora at least since 2012.
...
4. I renamed the `bfb` package that contains the Qt GUI to `bfb-core` as
that appears to be how the general population refers to it, in contrast to
`bfb-xt` or `bfb-classic`. I wanted to make sure the general population
